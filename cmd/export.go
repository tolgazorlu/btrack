...
func exportCSV(sessions []*db.Session, out *os.File) error {
	w := csv.NewWriter(out)
	_ = w.Write([]string{"id", "date", "start_time", "end_time", "duration_minutes", "task", "project", "message", "tags", "git_branch", "git_repo"})
	for _, s := range sessions {
		r := sessionToRow(s)
		_ = w.Write([]string{
			fmt.Sprintf("%d", r.ID),
			r.Date, r.StartTime, r.EndTime, r.DurationMinutes,
			r.Task, r.Project, r.Message, strings.Join(r.Tags, " "),
...
func init() {
	...
	exportCmd.Flags().StringP("project", "p", "", "export only sessions of this project")
